...
This conclusion is also supported by the mean absolute error, mean squared error, and $R^2$ score, which are shown in \cref{fig:mean-absolute-errors,fig:mean-squared-errors,fig:r2-scores}, with the exception of the volume target.
Note that here the \mbox{tree-based} approach only ranks second behind the \mbox{invariant-based} approach.
Its advantage on the three topological targets is expected by construction, as predicting them reduces to counting the nodes of the input graph (see \cref{tab:encoding}).

For the volume target, the invariant representation is strictly scale-invariant and carries no absolute size information, so the high $R^2$ cannot be attributed to the input encoding (see \cref{tab:encoding}).
As a reference, a trivial baseline that predicts the median volume of the true class from the training split already reaches $R^2 = 0.739$, outperforming all approaches except the \mbox{invariant-based} one.
Part size is thus largely inferable from part class within FabWave.
This also shows that $R^2$ discriminates poorly for this target, as it is dominated by a few large parts.
In terms of relative error (see \cref{fig:error-distributions}), which is not affected by this imbalance, the \mbox{tree-based} approach leads on the volume target as well, with a median of $0.23$ against $0.64$ for the \mbox{invariant-based} approach and $0.79$ for the trivial baseline.
On the topological targets, the \mbox{invariant-based} approach attains slightly lower medians, but with substantially heavier tails. Its mean relative errors exceed those of the \mbox{tree-based} approach by roughly a factor of seven. The \mbox{tree-based} approach therefore remains the most reliable across all regression targets.
...
For each violinplot, the corresponding boxplot is shown as well. The whiskers of the boxplot extend to 1.5 times the interquartile range. The tree-based approach achieves achieves the lowest mean relative error on all targets.}
