...
class CAD_Converter:
    ...
    def __init__(self, step_file: Path, skip_existing: bool = False):
        logging.debug(f"Initialisiere CAD_Converter with file: {step_file}")
    ...
    @classmethod
    def from_step(cls, step_file: Union[str, Path]) -> "CAD_Converter":
        return cls(Path(step_file))
    ...
    def to_stl(self, output_path: Union[str, Path]):
        ...
        if output_path.exists() and self._skip_existing:
            logger.debug(f"STL-Datei existiert bereits und wird übersprungen: {output_path}")
            return

        logger.debug(f"Konvertiere {self._step_file} zu STL...")
        ...
        logger.debug(f"STL gespeichert unter {output_path}")
    ...
    def to_ply(self, output_path: Union[str, Path]):
        ...
        if output_path.exists() and self._skip_existing:
            logger.debug(f"PLY-Datei existiert bereits und wird übersprungen: {output_path}")
            return
        ...
        logger.debug(f"Konvertiere STL zu PLY für {self._file_name}...")
        ...
        logger.debug(f"PLY gespeichert unter {output_path}")
    ...
    def to_vecset(self):
        ...
        logging.debug("init tmp dir for VecSet conversion")
        self._tmp_dir.mkdir(parents=True, exist_ok=True)
        tmp_ply = self._tmp_dir / f"tmp.ply"
        self.to_ply(tmp_ply)

        logger.debug(f"Konvertiere PLY zu VecSet für {self._file_name}...")
        ...
        return outputs['x'].squeeze(0).cpu().numpy()

            #np.save(output_path.as_posix(), outputs['x'].squeeze(0).cpu().numpy())
            #logger.debug(f"VecSet gespeichert unter {output_path}")
