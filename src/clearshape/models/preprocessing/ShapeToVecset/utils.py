...
def fps(pos, batch=None, ratio=0.5, random_start=True):
    """
    Farthest Point Sampling (simple CPU implementation)
    :param pos: Tensor of shape [N, D] (points in space)
    :param batch: Tensor of shape [N] (batch indices), or None for single batch
    :param ratio: float, fraction of points to sample
    :param random_start: bool, if True start from random point, else start from first
    :return: Tensor of indices of selected points
    """
    if batch is None:
    ...
    for b in batch.unique():
        ...
        selected = []
        distance = torch.full((N,), float('inf'), device=pos.device)

        if random_start:
            farthest = torch.randint(0, N, (1,), device=pos.device).item()
        else:
            farthest = 0
        ...
        selected = torch.where(mask)[0][selected]
...
class Attention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None, window_size=-1):
        ...
        out = F.scaled_dot_product_attention(
            q.transpose(1, 2),  # [B, H, N_q, D]
            kv[:, :, 0].transpose(1, 2),  # Keys
            kv[:, :, 1].transpose(1, 2),  # Values
            attn_mask=mask
        )

        out = out.transpose(1, 2)  # [B, N, H, D]
        out = rearrange(out, 'b n h d -> b n (h d)')  # [B, N, H*D]
